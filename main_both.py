...
from src.data_loader import (
    ...
    filter_valid_samples, build_image_caption_mapping, build_image_path_mapping
)
...
from src.ranker_both import BothRanker
from src.metrics import compute_ndcg


class FusedEmbedding:
    def __init__(self, st_model, clip_model):
        self.st_model = st_model
        self.clip_model = clip_model

    def get_embedding(self, input_data, embedding_type='text'):
        if embedding_type == 'text':
            return self.st_model.get_embedding(input_data)
        elif embedding_type == 'image':
            return self.clip_model.get_embedding(input_data, embedding_type='image')
        return None


def main():
    print("=" * 80)
    print("CAPTION + IMAGE (FUSED) RANKING")
    print("=" * 80)
    ...
    st_model = EmbeddingModel(ST_MODEL, DEVICE)

    print("Loading CLIP model...")
    clip_model = CLIPEmbeddingModel(DEVICE)

    fused_embedding = FusedEmbedding(st_model, clip_model)

    print("Generating contrast vectors...")
    contrast_vecs, context_vecs = generate_contrast_vectors(paraphrase_data, st_model.get_embedding)
    ...
    def create_data(samples):
        ...
        for row in samples:
            try:
                ...
                sent_emb = st_model.get_embedding(row['sentence'])
                if sent_emb is None:
                    continue
    ...
    print("\n" + "=" * 80)
    print("IMAGE RANKING (CAPTION + IMAGE FUSED)")
    print("=" * 80)
    image_to_caption = build_image_caption_mapping(dataset_df)
    ...
    ranker = BothRanker(image_to_caption, image_to_path, fused_embedding.get_embedding, id_tokens)
    ...
    print("\n" + "=" * 80)
    print("FINAL RESULTS (CAPTION + IMAGE FUSED)")
    print("=" * 80)
