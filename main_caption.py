...
from src.data_loader import (
    ...
    filter_valid_samples, build_image_caption_mapping
)
...
from src.ranker_caption import CaptionOnlyRanker
from src.metrics import compute_ndcg

def main():
    print("="*80)
    print("CAPTION-ONLY RANKING")
    print("="*80)
    ...
    embedding_model = EmbeddingModel(ST_MODEL, DEVICE)

    print("Generating contrast vectors...")
    contrast_vecs, context_vecs = generate_contrast_vectors(paraphrase_data, embedding_model.get_embedding)
    ...
    def create_data(samples):
        ...
        for row in samples:
            try:
                ...
                sent_emb = embedding_model.get_embedding(row['sentence'])
                if sent_emb is None:
                    continue
    ...
    print("\n" + "="*80)
    print("IMAGE RANKING (CAPTION-ONLY)")
    print("="*80)
    ...
    ranker = CaptionOnlyRanker(image_to_caption, embedding_model.get_embedding, id_tokens)
    ...
    print("\n" + "="*80)
    print("FINAL RESULTS (CAPTION-ONLY)")
    print("="*80)
