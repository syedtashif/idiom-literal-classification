...
class BothRanker:
    def __init__(self, image_to_caption, image_to_path, embedding_func, id_tokens=None):
    ...
    def rank_images(self, test_samples, token_to_token, context_vecs):
        ...
        print("Ranking with CAPTION + IMAGE (Fused)...")

        for test_sample in tqdm(test_samples, desc="Fused Ranking"):
            try:
                ...
                for img_name in expected_order:
                    ...
                    if img_name_lower in self.image_to_caption:
                        ...
                        caption_emb = self.embedding_func(filtered_caption, embedding_type='text')
                        if caption_emb is not None:
                            caption_sim = float(np.dot(ranking_emb, caption_emb))
                    ...
                    if img_name_lower in self.image_to_path:
                        ...
                        if img_emb is not None:
                            image_sim = float(np.dot(ranking_emb, img_emb))

                    fused_sim = self.caption_weight * caption_sim + self.image_weight * image_sim

                    if fused_sim > 0:
                        image_sims.append((os.path.basename(img_name), fused_sim))
